...
contract DEX is ERC20Base {
    ...
    constructor(address _token, address _defaultAdmin, string memory _name, string memory _symbol) ERC20Base () {
    ...
    function addLiquidity(uint256 _amount) public payable returns(uint256) {
        ...
        ERC20Base _toen = ERC20Base(token);
        ...
        if(tokenReserve == 0) {
            ...
            uint256 reserverEth = balanceInEth - msg.value;
            require (
                _amount >= (msg.value * tokenReserve) / reseveEth, 
                "Amount of tokens sent is less thant the minimum tokens required"
            );
            _token.transferFrom(msg.sender, address(this), _amount);
            unchecked {
                 _liquidity = (totalSupply() * msg.value) / reserverEth;
            }
    ...
    function getAmountOfTokens(
        ...
        uint256 demoninator = (inputReserve * 100) + inputAmount;
        unchecked {
            reutnr numerator / demoninator;
        }
    }
